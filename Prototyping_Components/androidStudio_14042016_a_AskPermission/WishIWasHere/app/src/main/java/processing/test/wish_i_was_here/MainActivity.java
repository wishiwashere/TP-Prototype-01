...
*/
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.CAMERA)) {
            Log.d("Main Activity", "Looking for a camera permission?");
        }
        else if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            Log.d("Main Activity", "Looking for a storage permission?");
        }else {
            Log.d("Main Activity", "Did I run ?");
        ...
        switch (requestCode) {
            case REQUEST_MULTIPLE_PERMISSIONS: {
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Log.d("Main Activity", "Permission was denied from device");
                }
                return;
            }
            default: {
                Log.d("Main Activity", "Laura said to put anything so Anything !");
                break;
            }
        }
    }
