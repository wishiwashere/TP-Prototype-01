...
// The current length of the tweet, this variable will be used to check if the users 
// tweet is within the character limit
int tweetLength = 0;

// File that will hold the path the image, currently wont work as the image is a pImage and file doesnt 
// mixing the two so either I need to figure out how to convert a pImage into a file stype or someway 
// access the storage, find out the last one and use that one
File imageFile = new File("");

//Used to hold the users message typed into the twitter input box
