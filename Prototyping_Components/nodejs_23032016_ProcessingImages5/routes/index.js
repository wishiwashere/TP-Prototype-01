...
router.get('/', function (req, res, next) {
    ...
    // Calling the HD version of removing the green screen
    removeGreenScreenHD();
...
function removeGreenScreenSD() {
    ...
    Jimp.read("./girlGreenScreen.jpg", function (err, cameraImage) {
        ...
        finishImage(keyedImage, "KeyedImage-SD.png");
    });
}

// Lower Quality Result - Takes approximatley 8 seconds to process
function removeGreenScreenHD() {
    // Using the Jimp package to read in an image
    Jimp.read("./girlGreenScreen.jpg", function (err, cameraImage) {
        ...
        finishImage(keyedImage, "KeyedImage-HD.png");
    });
}

// Composite the image (if necessary) and save to the server as a PNG
function finishImage(finalImage, filename){
    ...
    withBlackBackground.write(filename, function () {
