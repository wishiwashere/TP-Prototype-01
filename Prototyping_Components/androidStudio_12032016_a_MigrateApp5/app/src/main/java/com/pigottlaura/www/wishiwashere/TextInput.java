...
public class TextInput extends ClickableElement {
    ...
    public TextInput(PApplet _p, double x, double y, double w, double h, int col, String title) {
        ...
        this(_p, x, y, w, h, col, title, false, "LEFT");
    }

    // This partial constructor is used by text inputs that do not require their contents
    // to be blocked out i.e. any text input that is not a password
    public TextInput(PApplet _p, double x, double y, double w, double h, int col, String title, String align) {

        // Passing the relevant parametres to the main constructor. Since a password value
        // has not been included, passing false for this argument (i.e. assuming this in not
        // a password textInput
        this(_p, x, y, w, h, col, title, false, align);
    }

    // This partial constructor is used by text inputs that do not require their contents
    // to be blocked out i.e. any text input that is not a password
    public TextInput(PApplet _p, double x, double y, double w, double h, int col, String title, Boolean password) {

        // Passing the relevant parametres to the main constructor. Since a password value
        // has not been included, passing false for this argument (i.e. assuming this in not
        // a password textInput
        this(_p, x, y, w, h, col, title, password, "LEFT");
    }

    // This constructor is used by all text inputs
    public TextInput(PApplet _p, double x, double y, double w, double h, int col, String title, Boolean password, String align) {

        // Passing the relevant parametres from the constructor into the constructor
        // of the super class (ClickableElement)
        super(_p, x, y, w, h, col, title);
        ...
        textX1 = (float)(x - (w * 0.48));
        textY1 = (float)(y - (h * 0.45));
        textX2 = (float)(x + (w * 0.48));
        textY2 = (float)(y + (h * 0.45));
  ...
  /*-------------------------------------- showTextInput() ------------------------------------------------*/
    ...
    public void showTextInput() {
        ...
        if (mousePressed) {
            if (this.checkMouseOver()) {
                ...
                p.println("The " + inputTitle + " text input was clicked on");
            }
        }
    }
    ...
    private void addTextBox(String displayText) {
        p.rectMode(CORNERS);
        p.fill(0);
        p.textAlign(LEFT, textVertAlign);
        p.text(displayText, textX1, textY1, textX2, textY2);
    }
}