...
public class Icon extends ClickableElement {
  ...
  /*-------------------------------------- Constructor() ------------------------------------------------*/
    ...
    public Icon(PApplet _p, double x, double y, PImage img, String title, Boolean showTitle) {
        ...
        this(_p, x, y, _p.width * 0.15, _p.width * 0.15, img, title, showTitle, "Middle", "");
    }
    ...
    public Icon(PApplet _p, double x, double y, PImage img, String title, Boolean showTitle, String linkTo) {

        // If no width or height specified, defaulting these to 15% of the screen width.
        // If no link is specified, then defaulting this to an empty string.
        // Then passing this default, along with the specified parametres, into the
        // full constructor of this class
        this(_p, x, y, _p.width * 0.15, _p.width * 0.15, img, title, showTitle, "Middle", linkTo);
    }

    // This constructor is used by icons such as the homeIcon, that want to accept the default
    // width and height of an icon, and also link to another page
    public Icon(PApplet _p, double x, double y, PImage img, String title, Boolean showTitle, String titlePosition, String linkTo) {

        // If no width or height specified, defaulting these to 15% of the screen width.
        // If no link is specified, then defaulting this to an empty string.
        // Then passing this default, along with the specified parametres, into the
        // full constructor of this class
        this(_p, x, y, _p.width * 0.15, _p.width * 0.15, img, title, showTitle, titlePosition, linkTo);
    }

    // Partial Constructor
    public Icon(PApplet _p, double x, double y, double w, double h, PImage img, String title, Boolean showTitle, String linkTo){
        this(_p, x, y, w, h, img, title, showTitle, "Middle", linkTo);
    }
    ...
    public Icon(PApplet _p, double x, double y, double w, double h, PImage img, String title, Boolean showTitle, String titlePosition, String linkTo) {

        // Passing the relevant parametres from the constructor into the constructor
        // of the super class (Rectangle)
        super(_p, x, y, w, h, img, title);

        p = _p;

        p.println("icon width = " + w + "; " + title);
            ...
            if (this.checkMouseOver()) {
                ...
                if (this.iconLinkTo.length() > 0)
                {
                    ...
                    if (this.iconLinkTo.indexOf("http") > -1)
                    {
                        ...
                        p.link(this.iconLinkTo);

                        // Logging out what site the app will now be taken to
                        p.println("Going to " + this.iconLinkTo);
                    } else if (this.iconLinkTo.indexOf("_") == 0) {
                        p.println("CAM - about to change callfunction to " + this.iconLinkTo);
                        setCallFunction(this.iconLinkTo);
                        println("CAM Success = " + getCallFunction());
                        ...
                        println("CAM Success2 = " + getCallFunction());
                        // Logging out what page the app will now be taken to
                        p.println("Calling the " + this.iconLinkTo + "() function");
                    } else if (this.iconLinkTo.indexOf("-") == 0) {
                        setCurrentScreen(returnTo);
                        // Logging out what page the app will now be taken to
                        p.println("Returning to the " + this.iconLinkTo + "screen");
                    } else {
                        if(this.iconTitle.indexOf("Random") == 0){
                        }

                        p.println("ICON - about to callsetCurrentScreen - " + this.iconLinkTo);
                        ...
                        setCurrentScreen(this.iconLinkTo);

                        // incremented if the about screen was scrolled
                        setScreenTitleY(p.height * 0.08);

                        p.println("ICON - ScreenTitleY reset");
                        ...
                        p.println("ICON - about to reset screens");
                        resetScreens();
                        p.println("ICON - screens reset");

                        // Logging out what page the app will now be taken to
                        p.println("Going to the " + this.iconLinkTo);
                    }
                    p.println("Finished showing icon " + this.iconTitle);
                }
            }
        }
    }
