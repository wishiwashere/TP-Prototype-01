...
public class AboutScreen extends Screen {
    ...
    public Boolean aboutLoaded = false;

    private float iconTopY;
    private float iconBottomY;
    private float appWidth;
    private float appHeight;
    private float iconRightX;

    // Creating a public constructor for the AboutScreen class, so that
    // an instance of it can be declared in the main sketch
    public AboutScreen(PApplet _p, PImage bgImage, float _appWidth, float _appHeight, float _iconRightX, float _iconTopY, float _iconBottomY, PImage homeIconImage, PImage facebookAccountIconImage, PImage twitterAccountIconImage, PImage instagramAccountIconImage) {

        // Passing the color parametre to the super class (Screen), which will in
        // turn call it's super class (Rectangle) and create a rectangle with the
        // default values i.e. fullscreen, centered etc.
        super(_p, _appWidth/2, _appHeight/2, _appWidth, _appHeight * 2, bgImage);

        p = _p;

        iconRightX = _iconRightX;
        iconTopY = _iconTopY;
        iconBottomY = _iconBottomY;
        appWidth = _appWidth;
        ...
        Icon homeIcon = new Icon(p, iconRightX, iconTopY, homeIconImage, "Home", false, "HomeScreen");
        Icon facebookIcon = new Icon(p, appWidth * 0.2, iconBottomY, facebookAccountIconImage, "Facebook", false, "https://www.facebook.com/wishiwashereapp");
        Icon twitterIcon = new Icon(p, appWidth * 0.5, iconBottomY, twitterAccountIconImage, "Twitter", false, "https://twitter.com/wishiwashere");
        Icon instagramIcon = new Icon(p, appWidth * 0.8, iconBottomY, instagramAccountIconImage, "Instagram", false, "https://www.instagram.com/wishiwashereapp/");
    ...
    public void showScreen(float previousMouseY) {
        if (!this.aboutLoaded) {
        ...
        this.addImage(p.loadImage("aboutPageTeamImage.jpg"), appWidth/2, this.getY() + (appHeight * -0.25), appWidth * 0.7, appHeight * 0.2);

        p.rectMode(CORNER);
        p.textAlign(LEFT);
        p.textSize((float)(appWidth * 0.05));

        p.text(aboutText, (float) (appWidth * 0.1), (float)(this.getY()  + (appHeight * -0.1)), (float)(appWidth * 0.8), (float)(appHeight * 0.9));
