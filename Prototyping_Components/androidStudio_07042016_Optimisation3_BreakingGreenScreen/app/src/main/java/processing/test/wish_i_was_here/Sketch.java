...
public class Sketch extends PApplet {
    ...
    /*-------------------------------------- Saving ------------------------------------------------*/
        ...
        currentImage = createImage(appWidth, appHeight, ARGB);
        currentImage.loadPixels();
        for (int i = 0; i < currentImage.pixels.length; i++) {
            currentImage.pixels[i] = color(255, 255, 255, 0);
        }
    ...
    public void previewGreenScreen() {
        ...
        try {
            ...
            for (int i = 0; i < keyedImage.pixels.length; i++) {
                ...
                float pixelHue = hue(keyedImage.pixels[i]);

                // If the hue of this pixel falls anywhere within the range of green in the colour spectrum
                if (pixelHue > 60 && pixelHue < 180) {

                    float pixelSaturation = saturation(keyedImage.pixels[i]);
                    float pixelBrightness = brightness(keyedImage.pixels[i]);
            ...
            // Updating the pixel arrays of the the keyed image
