...
public class Sketch extends PApplet {
    ...
    */
    ...
    public void sendCameraImageToServer(){
        ...
        if (ketaiCamera.savePhoto("serverImage.jpg")){
        ...
        try{
            PostRequest postReq = new PostRequest("http://wishiwashere.azurewebsites.net/");
            postReq.addFile("GreenImage", serverImage);
            postReq.addData("name", "laura");
            println("IMG2SERVER - About to send server request");
            postReq.send();
            println("IMG2SERVER - Post request sent");

            //println("IMG2SERVER - Response - " + postReq.getContent());
            //println("IMG2SERVER - Response Headers - " + postReq.getHeader("Content-type"));


            //currentImage = loadImage(postReq.getContent());
            //println("IMG2SERVER - currentImage set to loaded image");
        } catch(Exception e){
            println("IMG2SERVER - unable to send post request - " + e);
        }
        currentImage = loadImage("https://wishiwashere.azurewebsites.net/images/KeyedImage.png");
        println("IMG2SERVER - currentImage set to loaded image");
        readingImage = false;
    }
    /*
